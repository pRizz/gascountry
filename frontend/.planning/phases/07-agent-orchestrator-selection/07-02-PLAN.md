...
import type { Orchestrator } from "@/api/types";
```
2. Update PromptInputProps interface:
```typescript
...
```
4. Update MainPanel's onStartSession prop call to include orchestrator - trace through MainPanel to ensure it passes orchestrator from PromptInput

Note: MainPanel likely wraps PromptInput. Check its props and ensure orchestrator flows from PromptInput through MainPanel to Index.tsx's handleStartSession.
  </action>
  <verify>
...
Creating a session with ralph orchestrator works end-to-end.
  ...
  <done>
OrchestratorSelector appears in PromptInput UI. User can select Ralph (default). GSD/Gastown are disabled with badge. Selected orchestrator is passed to createSession API.
  </done>
</task>
